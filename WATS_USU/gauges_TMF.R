...
###      USGS stream data retrieved from: http://       ###
###                Required files for this script                 ###
###                "gauges_WEAPNodes.csv", "WEAP_###
###                                                                                      ###
############################################################################################
## 
##
...
############################################################################################
###                                                                                     ###
###          Estimate Total Monthy Flow from observed daily averages (USGS)            ###
...
for (i in seq_along(Tot_flow_month)) {
  # include only the columns for the date ("YearMonth") and total cubic meters ("TotCM")
    ss = as.data.frame(Tot_flow_month[[i]][,c("YearMonth","TotCF")]) 
...
write.csv(singlesheet, "./output_R/TotalFlow_Month_all.csv", row.names = FALSE, na = "")
# do log version of this eventually
write.csv(singlesheet, "./output_R/TotalFlowLog_Month_all.csv", row.names = FALSE, na = "")
...
# write.csv(modelyears, "./output_R/myTotalFlowLog_Month_all.csv", row.names = FALSE)
write.csv(modelyears, "./output_R/myTotalFlow_Month_all.csv", row.names = FALSE)
