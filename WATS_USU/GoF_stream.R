...
weapgof <- function(filename, title) {
  ...
  colnames(ove)[1] <- "site"
  ove <- as.data.frame(ove)
  ove1 <- merge(as.data.frame(ove), subset(ebins,select=-gage), by.x = "site", "site_name_WEAP")
  refcols <- c("site", "Basin", "elev_m", "bins", "n")
  ove1 <- ove1[, c(refcols, setdiff(names(ove1), refcols))]
  write.csv(ove1, paste("./output_R/GoF/GoodnessofFit_", title, "_", ".csv", sep = ""), row.names = FALSE)
  ## plotting the data
  y.range=range(c(as.list(twest), as.list(tgage)), na.rm=T)
  y.range[2]=y.range[2]/1000000
  gauges2plot <- names(tgage)
  YrMon <- as.character(row.names(tgage))
  for (i in seq_along(gauges2plot)) {
    WEAP <- twest[ ,which(names(twest) == gauges2plot[i])]/1000000 # convert CFS to MCFS
    USGS <- tgage[ ,i]/1000000 # convert CFS to MCFS
    ...
    plot(range(WEAPvsUSGS$YrMon), y.range, type='n', xlab = "", ylab = "Total Flow [Millions CF/month]", main = paste("WEAP estimate vs ", ID, sep = ""))
  ...
  for (i in seq_along(gauges2plot)) {
    ...
    plot(range(WEAPvsUSGS$YrMon), y=c(1,y.range[2]), type='n', xlab = "", log = "y", ylab = "Total Flow [Million CFS]", main = paste("WEAP estimates vs ", ID, "\non a log10 scale",sep = ""))
    lines(WEAPvsUSGS$YrMon, WEAPvsUSGS$WEAP, col="red")
    lines(WEAPvsUSGS$YrMon, WEAPvsUSGS$USGS, col="blue")
    legend("topright",
           c("Simulated","Observed"), 
...
GoF_tables <- function(dirloc) {
  ...
  for (i in seq_along(GoF_files)) {
    ...
    sapply(names(z), function (zz) write.csv(z[[zz]], file=paste("output_R/",zz, "csv", sep="."), row.names = FALSE))
  }
  final <- do.call(cbind, z)
  write.csv(final, "output_R/all.csv", row.names = FALSE)
}
