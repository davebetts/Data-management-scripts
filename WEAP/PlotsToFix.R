...
###     !!!                                                                                 ###
###     !!!      Processing USGS stream gaugeparison           ###
###     !!! USGS stream data retrieved from: ustory       ###
...
library(zoo)
a <- gauges[,1:2]
colnames(a) <- c("YearMon", "flow")
a$YearMon <- as.yearmon(a$YearMon, format = "%Y%m")
b <- weap[,1:2]
colnames(b) <- c("YearMon", "flow")
a$simobs <- "USGS"
b$simobs <- "WEAP"
d <- rbind(a, b)
colnames(d) <- c("YearMon", "flow", "simobs")

a = read.csv("a.csv")
library(plotly)
p <- plot_ly(a, x = YearMon, y = flow)  %>% add_trace(y = YearMon + 5, name = "USGS", line = list(shape = "spline")) %>% plot_ly(b, x = YearMon, y = flow)  %>% add_trace(y = y + 5, name = "WEAP", line = list(shape = "spline"))
p

plot_ly(a, x = YearMon, y = flow)  %>% add_trace(y = YearMon, name = "USGS", line = list(shape = "spline"))


library(ggplot2)
ggplot(d, aes(YearMon, flow)) +
  geom_line(aes(colour = simobs), size = 1) +
  labs(title = "Smaller population = Greater variation", x = "Date", y = "flow [some volume]") +
  theme_bw() +
...
# can log transform in qplot with: log = "x"
...
for (i in seq_along(gaugeIDs)) {
  ...
  plot(WEAPvsUSGUS[,1]~WEAPvsUSGS[,2:3], col = c("red", "blue"), type = "l", xlab = "Date", ylab = "Total Flow", main = paste("WEAP output vs USGS", gaugeIDs[i])
       ...
       # the values match the YearMonth labels that will appear in other tables
