...
weap <- read.csv("WEAP85to05.csv", check.names = FALSE)
gauges <- read.csv("myTotalFlow_Month_all.csv", check.names = FALSE)
head(weap)
head(gauges)
...
for (i in seq_along(gaugeIDs)) {
    ...
    mainDir <- "."
    subDir <- paste("sim_vs_obs_plots", format(Sys.time(), "%Y_%m_%d_%H%M%S"),sep = "_")
